var url = 'http://121.42.29.124/workoa/index.php?s=/Api/Renwu/sendRenwu';
...
function createUploader() {
	task = plus.uploader.createUpload(url, {
		...
		if(status == 200) {
			...
			var page = plus.webview.getWebviewById('view/task/task.html');
...
finish.addEventListener('tap', function() {
	...
	task.addData('fabuzheid', this_phoneNum);
	task.addData('content', content);
	task.addData('type',taskType);
	task.addData('finishdate',date);
	task.addData('jieshouzheid',jieshouPeople);
	
	task.addData('date', date);
