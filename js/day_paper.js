 ...
 * 下拉具体业务的实现
 */
function pullDownGetData(keyword) { 
			...
			if(data.flag == 200) { 
				...
				for(var i = 0; i < result.length; i++) {
					...
					li.innerHTML = '<div class="first_line"><img class="headImg" src="' + result[i].personlogo + '" onerror="imgerror(this)"/>' +
						...
						'<div class="second">' +
						'<span class="line"><span id="yiwancheng" class="title">已完成</span>' + result[i].finishwork + '</span>' +
						'<span class="line"><span id="weiwancheng" class="title">未完成</span>' + result[i].unfinishwork + '</span>' +
						'<span class="line"><span id="xuxietiao" class="title">需协调</span>' + result[i].needhelp + '</span>' +
						'<span class="line "><span id="beizhu" class="title">备注</span>' + result[i].moveinfo + '</span>' +
						'</div>';
 ...
 * 上拉加载具体业务实现
 */
function pullUpGetData(keyword) {
	var table = document.body.querySelector('.mui-table-view');
	thePage++;
	//这个接口有问题，返回值是300   郭梦娜-2016-07-25-获得公司日报接口-		
	mui.ajax('http://121.42.29.124/workoa/index.php?s=/Api/Ribao/getribao/', {
